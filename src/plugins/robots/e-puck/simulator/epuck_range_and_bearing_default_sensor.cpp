...
CEPuckRABDefaultSensor::CEPuckRABDefaultSensor() :
    ...
    m_pcRangeAndBearingEquippedEntityDefault(NULL),
...
void CEPuckRABDefaultSensor::SetRobot(CComposableEntity& c_entity) {
    ...
    printf("Inside Defualt EPuck RAB Sensor");
...
void CEPuckRABDefaultSensor::Init(TConfigurationNode& t_tree) {
    // *************** RSM My Implementation of Sensor**********************
     try {
         /* Parent class init */
         ...
         m_pcRangeAndBearingEquippedEntityDefault->SetMedium(*m_pcRangeAndBearingMedium);
      }
...
void CEPuckRABDefaultSensor::Update() {
      ...
      const CSet<CRABEquippedEntity*,SEntityComparator>& setRABsDefault = m_pcRangeAndBearingMedium->GetRABsCommunicatingWith(*m_pcRangeAndBearingEquippedEntityDefault);
      ...
      for(CSet<CRABEquippedEntity*>::iterator it = setRABsDefault.begin();
          it != setRABsDefault.end(); ++it) {
         ...
         if(m_pcRNG == nullptr || /* No noise to apply */
            ...
            if(m_bShowRays) {
               m_pcControllableEntity->AddCheckedRay(false,
                                                     CRay3(cRABEntity.GetPosition(),
                                                           m_pcRangeAndBearingEquippedEntityDefault->GetPosition()));
            }
            /* Calculate vector to entity */
            cVectorRobotToMessage = cRABEntity.GetPosition();
            cVectorRobotToMessage -= m_pcRangeAndBearingEquippedEntityDefault->GetPosition();
             ...
             */
            cVectorRobotToMessage.Rotate(m_pcRangeAndBearingEquippedEntityDefault->GetOrientation().Inverse());
...
//  void CEPuckRABDefaultSensor::Enable() {
//      m_pcRangeAndBearingEquippedEntityDefault->Enable();
//      CCI_Sensor::Enable();
//    }
