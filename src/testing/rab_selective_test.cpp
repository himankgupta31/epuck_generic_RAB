...
CEPuckController::CEPuckController() : m_uRobotID(0),
...
void CEPuckController::Init(TConfigurationNode &t_node)
{
    ...
    try
    {
        m_pcRABActuatorDefault = GetActuator<CCI_RangeAndBearingActuator>("range_and_bearing");
    }
    catch (CARGoSException &ex)
    {
    }
...
void CEPuckController::ControlStep()
{
    ...
    if (m_pcRABS != NULL)
    {
        ...
        CByteArray x;
        x.operator<<(2);
        // while (x.Size() < m_pcRABActuatorDefault->GetSize())
        //     x << static_cast<UInt8>(0);
        for (size_t i = 0; i < tPackets.size(); ++i)
        {
            LOG<<"RSM"<<"| "<<
                            tPackets[i].Range<< " "<<
                           ToDegrees(tPackets[i].HorizontalBearing).GetValue()<<" "<<
                           ToDegrees(tPackets[i].VerticalBearing).GetValue()<<" "<<
                            tPackets[i].Data<<"\n";
        }
    }
    ...
    //     while (x.Size() < m_pcRABActuatorDefault->GetSize())
    //         x << static_cast<UInt8>(0);
    //     m_pcRABActuatorDefault->SetData(x);
    //     for (size_t i = 0; i < packets.size(); ++i)
    //         if (packets[i]->Data[0] != m_uRobotID)
    //         {

    //             LOG << m_uRobotID << "| " << packets[i]->Range << " " << ToDegrees(packets[i]->Bearing).GetValue() << " " << packets[i]->Data[0] << " " << packets[i]->Data[1] << " " << packets[i]->Data[2] << " " << packets[i]->Data[3] << "\n";
    //         }
